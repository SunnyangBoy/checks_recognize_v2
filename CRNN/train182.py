    ...
    for epoch in range(config.epochs):
        ...
        if epoch % 3 == 0:
            ...
            torch.save(state_dict,
                       '/root/last_dataset/crnn_char_pths/catword_lr3_epoch_{}_acc{:4f}.pth'.format(epoch + 1,
                                                                                                       train_acc / train_acc_cnt))

        if train_acc / train_acc_cnt > 0.95:
            state_dict = model.state_dict()
            torch.save(state_dict,
                       '/root/last_dataset/crnn_char_pths/catword_lr3_epoch{}_acc{:4f}.pth'.format(epoch + 1,
                                                                                                      train_acc / train_acc_cnt))
...
if __name__ == '__main__':
    ...
    train_dataset_hand_word = dataset.BaseDataset(data_dir, train_infos_hand_word, transform=dataset.img_enhancer,
                                                  _type='hand_word')
    train_dataset_hand_num = dataset.BaseDataset(data_dir, train_infos_hand_num, transform=dataset.img_enhancer,
                                                 _type='hand_num')
    train_dataset_print_word = dataset.BaseDataset(data_dir, train_infos_print_word, transform=dataset.img_enhancer,
                                                   _type='print_word')
    train_dataset_print_num = dataset.BaseDataset(data_dir, train_infos_print_num, transform=dataset.img_enhancer,
                                                  _type='print_num')
    train_dataset_symbol = dataset.BaseDataset(data_dir, train_infos_symbol, transform=dataset.img_enhancer,
                                               _type='symbol')
    train_dataset_seal = dataset.BaseDataset(data_dir, train_infos_seal, transform=dataset.img_enhancer, _type='seal')
    train_dataset_catword = dataset.BaseDataset(data_dir, train_infos_catword, transform=dataset.img_enhancer,
                                                _type='catword')
