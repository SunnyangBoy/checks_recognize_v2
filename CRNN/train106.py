...
def train(model, criterion, converter, device, train_datasets, valid_datasets=None, pretrain=False):
    print('Device:', device)
    ...
    if pretrain:
        ...
        model.load_state_dict(
            torch.load('/root/last_dataset/crnn_word_pths/word_lr3_epoch_7_acc0.551955.pth'))

    dataset_name = 'print_word'
    batch_dict = {'print_word': 32, 'hand_num': 48, 'print_num': 48, 'symbol': 64, 'hand_word': 64, 'seal': 64,
                  'catword': 32}
    dataset = train_datasets.get(dataset_name)
    dataloader = DataLoader(dataset, batch_size=batch_dict.get(dataset_name), shuffle=True, num_workers=4,
                            drop_last=False)
    ...
    for epoch in range(config.epochs):
        ...
        if epoch % 3 == 0:
            ...
            print('========== valid acc: ', acc, '  ============valid loss: ', valid_loss)
        ...
        if epoch % 3 == 0:
            ...
            torch.save(state_dict,
                       '/root/last_dataset/crnn_word_pths/word_lr3_epoch_{}_acc{:4f}.pth'.format(epoch + 1,
                                                                                                       train_acc / train_acc_cnt))

        if train_acc / train_acc_cnt > 0.8:
            state_dict = model.state_dict()
            torch.save(state_dict,
                       '/root/last_dataset/crnn_word_pths/word_lr3_epoch{}_acc{:4f}.pth'.format(epoch + 1,
                                                                                                      train_acc / train_acc_cnt))
...
if __name__ == '__main__':
    ...
    train_infos_catword = train_img_infos[train_img_infos.type == '联合汉字']
    ...
    valid_infos_catword = valid_img_infos[valid_img_infos.type == '联合汉字']
    '''
    ...
    train_dataset_hand_word = dataset.BaseDataset(data_dir, train_infos_hand_word, transform=dataset.img_padder,
                                                  _type='hand_word')
    train_dataset_hand_num = dataset.BaseDataset(data_dir, train_infos_hand_num, transform=dataset.img_padder,
                                                 _type='hand_num')
    train_dataset_print_word = dataset.BaseDataset(data_dir, train_infos_print_word, transform=dataset.img_padder,
                                                   _type='print_word')
    train_dataset_print_num = dataset.BaseDataset(data_dir, train_infos_print_num, transform=dataset.img_padder,
                                                  _type='print_num')
    train_dataset_symbol = dataset.BaseDataset(data_dir, train_infos_symbol, transform=dataset.img_padder,
                                               _type='symbol')
    train_dataset_seal = dataset.BaseDataset(data_dir, train_infos_seal, transform=dataset.img_padder, _type='seal')
    train_dataset_catword = dataset.BaseDataset(data_dir, train_infos_catword, transform=dataset.img_padder,
                                                _type='catword')

    valid_dataset_hand_word = dataset.BaseDataset(data_dir, valid_infos_hand_word, transform=dataset.img_padder,
                                                  _type='hand_word')
    valid_dataset_hand_num = dataset.BaseDataset(data_dir, valid_infos_hand_num, transform=dataset.img_padder,
                                                 _type='hand_num')
    valid_dataset_print_word = dataset.BaseDataset(data_dir, valid_infos_print_word, transform=dataset.img_padder,
                                                   _type='print_word')
    valid_dataset_print_num = dataset.BaseDataset(data_dir, valid_infos_print_num, transform=dataset.img_padder,
                                                  _type='print_num')
    valid_dataset_symbol = dataset.BaseDataset(data_dir, valid_infos_symbol, transform=dataset.img_padder,
                                               _type='symbol')
    valid_dataset_seal = dataset.BaseDataset(data_dir, valid_infos_seal, transform=dataset.img_padder, _type='seal')
    valid_dataset_catword = dataset.BaseDataset(data_dir, valid_infos_catword, transform=dataset.img_padder,
                                                _type='catword')
    ...
    train(model, criterion, converter, device, train_datasets, valid_datasets, pretrain=True)
