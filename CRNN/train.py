...
def train(model, criterion, converter, device, train_datasets, valid_datasets=None, pretrain=False):
    ...
    if pretrain:
        ...
        model.load_state_dict(
            torch.load('/root/last_dataset/last_pths/catword_lr3_epoch_10_acc0.495342.pth'))

    dataset_name = 'catword'
    batch_dict = {'print_word': 32, 'hand_num': 48, 'print_num': 48, 'symbol': 64, 'hand_word': 64, 'seal': 64, 'catword': 32}
    ...
    for epoch in range(config.epochs):
        ...
        if epoch % 3 == 0:
            ...
            torch.save(state_dict, '/root/last_dataset/crnn_catword_pths/catword_lr3_epoch_{}_acc{:4f}.pth'.format(epoch + 1, train_acc/train_acc_cnt))

        if train_acc/train_acc_cnt > 0.7:
            state_dict = model.state_dict()
            torch.save(state_dict, '/root/last_dataset/crnn_catword_pths/catword_lr3_epoch{}_acc{:4f}.pth'.format(epoch + 1, train_acc/train_acc_cnt))
...
if __name__ == '__main__':
    ...
    train_dataset_hand_word  = dataset.BaseDataset(data_dir, train_infos_hand_word,  transform=dataset.img_padder, _type='hand_word')
    train_dataset_hand_num   = dataset.BaseDataset(data_dir, train_infos_hand_num,   transform=dataset.img_padder, _type='hand_num')
    train_dataset_print_word = dataset.BaseDataset(data_dir, train_infos_print_word, transform=dataset.img_padder, _type='print_word')
    train_dataset_print_num  = dataset.BaseDataset(data_dir, train_infos_print_num,  transform=dataset.img_padder, _type='print_num')
    train_dataset_symbol     = dataset.BaseDataset(data_dir, train_infos_symbol,     transform=dataset.img_padder, _type='symbol')
    train_dataset_seal       = dataset.BaseDataset(data_dir, train_infos_seal,       transform=dataset.img_padder, _type='seal')
    train_dataset_catword    = dataset.BaseDataset(data_dir, train_infos_catword,    transform=dataset.img_padder, _type='catword')
    ...
    valid_dataset_catword    = dataset.BaseDataset(data_dir, valid_infos_catword,    transform=dataset.img_padder, _type='catword')
