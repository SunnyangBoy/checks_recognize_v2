...
createImg_root = '/mnt/train_rotate90/images'
...
'''
def rev_rotate_image(w, h, angle, scale=1):
    # 角度变弧度
    rangle = np.deg2rad(angle)  # angle in radians
    # now calculate new image width and height
    nw = (abs(np.sin(rangle) * h) + abs(np.cos(rangle) * w)) * scale
...
def rev_rotate_ver(box, rot_mat):
    box = box.reshape(4, 2)
    point1 = np.dot(rot_mat, np.array([box[0, 0], box[0, 1], 1]))  # 获取原始矩形的四个中点，然后将这四个点转换到旋转后的坐标系下
    point2 = np.dot(rot_mat, np.array([box[1, 0], box[1, 1], 1]))
    point3 = np.dot(rot_mat, np.array([box[2, 0], box[2, 1], 1]))
...
if __name__ == '__main__':
    img_label_dir = '/mnt/data/datasource/train/labels'
    for root, dirs, files in os.walk(img_label_dir):
        for file in sorted(files):
            file_path = os.path.join(root, file)
            image_name = file[0: -4] + '.jpg'
            image_dir = os.path.join('/mnt/data/datasource/train/images', image_name)
            ...
            with open(lab_savepath, 'w') as writer:
                with open(file_path, 'r') as lines:
                    ...
                    for l, line in enumerate(lines):
                        ...
                        vertice = np.array(vertice)
                        if flag:
                            new_vertice = rotate_ver(vertice, rot_mat)
                            vertice = new_vertice
                        #print(vertice)
                        new_line = []
                        new_line.append(line[0])
                    ...
                    writer.close()
    '''
    image_name = 'normal_pufa_check_img_tm4_0008_bg.jpg'
    image_dir = '/mnt/test90/images/normal_pufa_check_img_tm4_0008_bg.jpg'
    img_savepath = os.path.join(createImg_root, 'test.jpg')
    print(img_savepath)
    lab_savepath = os.path.join(createLab_root, 'test.txt')
    print(lab_savepath)
    image = cv2.imread(image_dir)
    image, rot_mat = rotate_image(image, -90, scale=1)
    cv2.imwrite(img_savepath, image)
    with open(lab_savepath, 'w') as writer:
        with open('/mnt/test90/labels/normal_pufa_check_img_tm4_0008_bg.txt', 'r') as lines:
            lines = lines.readlines()
            for l, line in enumerate(lines):
                line = line.split(';')
                vertice = [int(vt) for vt in line[1:-1]]
