...
def valid(valid_loader, model, criterion, device):
    ...
    for i, (img, gt_score, gt_geo, ignored_map, vertices) in enumerate(valid_loader):
        ...
        if flag:
            image.save(os.path.join('/home/chen-ubuntu/Desktop/checks_dataset/tmp/', str(i) + '_1_detected' + '.jpg'))

        print('*'*10)'''


    print('valid acc is [{}/{}]'.format(corret, box_cnt))
    print('valid acc is: ', corret/box_cnt)
...
if __name__ == '__main__':
    img_path = os.path.abspath('/mnt/tvmerge_rotate180/images')
    gt_path = os.path.abspath('/mnt/tvmerge_rotate180/labels')
