...
def train(train_root_path, pths_path, batch_size, lr, num_workers, epoch_iter, interval):
    ...
    model.load_state_dict(torch.load('/root/checks_recognize_v2/pths/model_epoch_lr4_bat_stamp_68.pth'))
    ...
    for epoch in range(epoch_iter):
        ...
        if epoch % interval == 0:
            ...
            torch.save(state_dict, os.path.join(pths_path, 'east_model_{}_{:.4f}.pth'.format(epoch+1, epoch_loss/int(file_num/batch_size))))


if __name__ == '__main__':
    train_root_path = os.path.abspath('/mnt/tvmerge_rotate180')
    pths_path = '/root/last_dataset/east_seal_tmp_pth'
    batch_size = 8
    lr = 1e-4
    num_workers = 4
    epoch_iter = 1000
