...
def train(train_img_path, train_gt_path, pths_path, batch_size, lr, num_workers, epoch_iter, interval):
	...
	model.load_state_dict(torch.load('/root/last_dataset/east_tmp_pths/east_model_9_0.2783.pth'))
	data_parallel = False
	...
	for epoch in range(epoch_iter):	
		...
		if epoch % interval == 0:
			...
			torch.save(state_dict, os.path.join(pths_path, 'east_model_{}_{:.4f}.pth'.format(epoch+10, epoch_loss/int(file_num/batch_size))))


if __name__ == '__main__':
	train_img_path = os.path.abspath('/mnt/train_rotate180/images')
	train_gt_path  = os.path.abspath('/mnt/train_rotate180/labels')
	pths_path      = '/root/last_dataset/east_tmp_pths/'
	batch_size     = 8
	lr             = 1e-4
	num_workers    = 4
	epoch_iter     = 600
