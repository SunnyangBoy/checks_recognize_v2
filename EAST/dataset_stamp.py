...
class custom_dataset(data.Dataset):
    def __init__(self, root_path, scale=0.25, length=512):
        super(custom_dataset, self).__init__()

        img_one_path = os.path.join(root_path, 'images')
        img_one_files = [os.path.join(img_one_path, img_file) for img_file in sorted(os.listdir(img_one_path))]

        gt_one_path = os.path.join(root_path, 'labels')
        gt_one_files = [os.path.join(gt_one_path, gt_file) for gt_file in sorted(os.listdir(gt_one_path))]

        self.img_files = img_one_files
        self.gt_files = gt_one_files
        self.scale = scale
        self.length = length
