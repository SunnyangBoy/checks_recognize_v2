...
createImg_root = '/Users/zhuzhenyang/Desktop/rotated_mode1/Image/'
#createLab_root = '/Users/zhuzhenyang/Desktop/rotated_mode1/Label/'
...
def rotate_img(img_dir, img_savepath, file_path, flag):
    image = Image.open(img_dir)
    if flag:
        image = image.rotate(180, Image.BILINEAR)
    image.save(img_savepath)

    #with open(lab_savepath, 'a') as writer:
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('/root/last_dataset/tmp_pths/rotated_95_0.0003.pth'))
    model.to(device)
    model.eval()

    #img_label_dir = os.path.join(configs.img_rootdir, img_mode, 'Label')
    #print(img_label_dir)
    with open('/root/last_dataset/test_result/test_rotate180.txt', 'w') as writer:
        for root, dirs, files in os.walk('/mnt/test_rotate90/images'):#img_label_dir):
            for file in sorted(files):
                ...
                if output[0][0] < output[0][1]:
                    ...
                    writer.write(str(width) + ',' + str(height))
                    writer.write('\n')
                img_savepath = os.path.join('/root/last_dataset/test_result/test_rotate180', img_name)
                #lab_savepath = os.path.join(createLab_root, file)
                rotate_img(img_dir, img_savepath, file_path, flag)